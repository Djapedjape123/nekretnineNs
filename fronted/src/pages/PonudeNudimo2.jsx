...
import buildingsImage from '../assets/prvi2.webp'
import { t } from '../i1n8'
  ...
  return (
    <section className="relative py-24 bg-[#fcfcfc] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        >
          <span className="inline-block px-4 py-1.5 mb-6 text-sm font-bold tracking-widest text-yellow-600 uppercase bg-yellow-100 rounded-full">
            {t('premiumUsluga')}
          </span>
          ...
          <div className="space-y-6 text-lg text-gray-600 leading-relaxed mb-10">
            <p className="font-medium text-gray-800">{t('longTermTitle2')}</p>
            <p>{t('i1')}</p>
            <p>{t('i2')}</p>
            <p>{t('i3')}</p>
          </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {services.map((service, index) => (
                <div 
                  ...
                  className={`group flex items-center p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:border-yellow-300 transition-all duration-300 ${
          ...
          <div className={`mt-12 p-8 bg-gray-900 rounded-[2rem] text-white relative overflow-hidden transition-all duration-1000 delay-[800ms] ${
            loaded ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          }`}>
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-3xl">🎯</span>
                <h5 className="text-xl font-bold tracking-wide uppercase">{t('nasCilji')}</h5>
              </div>
